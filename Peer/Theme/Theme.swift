...
struct PeerButtonStyle: ButtonStyle {
    var foreground: Color = .white
    var background: Color = PeerTheme.accentCoral
    var border: Color = PeerTheme.accentCoral.opacity(0.2)
    ...
    func makeBody(configuration: Configuration) -> some View {
        configuration.label
            ...
            .overlay(
                RoundedRectangle(cornerRadius: PeerTheme.cardCornerRadiusSmall)
                    .strokeBorder(border, lineWidth: 0.5)
            )
            .clipShape(RoundedRectangle(cornerRadius: PeerTheme.cardCornerRadiusSmall))
...
struct SectionHeaderView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 4) {
            Text(title)
                .font(PeerTheme.titleFont(PeerTheme.titleMedium, weight: .bold))
...
struct PeerBackgroundView: View {
    var body: some View {
        PeerTheme.background.ignoresSafeArea()
    }
}

extension View {
    func peerScreenBackground() -> some View {
        background {
            PeerBackgroundView()
        }
    }

    func peerCardSurface(cornerRadius: CGFloat = PeerTheme.cardCornerRadius) -> some View {
        background(PeerTheme.cardGradient)
            .clipShape(RoundedRectangle(cornerRadius: cornerRadius))
            .overlay(
                RoundedRectangle(cornerRadius: cornerRadius)
                    .strokeBorder(PeerTheme.cardBorder, lineWidth: 1)
            )
            .shadow(
                color: .black.opacity(PeerTheme.cardShadowOpacity),
                radius: PeerTheme.cardShadowRadius,
                x: 0,
                y: PeerTheme.cardShadowY
            )
    }
}

struct PeerMosaicView: View {
    let seed: String
    ...
    private var resolvedURLs: [URL] {
        ...
        return PeerImageLibrary.mixed(seed: seed)
    }
    ...
    var body: some View {
        ...
        GeometryReader { geo in
            let safeScreenWidth = UIScreen.main.bounds.width - (PeerTheme.horizontalPadding * 2)
...
struct PeerStickyCTA<Leading: View>: View {
