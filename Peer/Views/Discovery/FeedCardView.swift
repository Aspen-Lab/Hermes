        ...
        HStack(spacing: 12) {
            if let onSave = onSave {
                Button(action: onSave) {
                    Label("Save", systemImage: "bookmark")
                        .font(PeerTheme.bodyFont(14, weight: .semibold))
                }
                .buttonStyle(
                    PeerButtonStyle(
                        foreground: PeerTheme.textPrimary,
                        background: Color.white,
                        border: PeerTheme.divider
                    )
                )
            }
            ...
            if let onNotInterested = onNotInterested {
                iconActionButton(
                    ...
                    foreground: PeerTheme.accentDismiss,
            ...
            if showMoreLikeThis, let onMoreLikeThis = onMoreLikeThis {
                iconActionButton(
                    ...
                    foreground: PeerTheme.accentMore,
                    background: PeerTheme.accentMoreLight
                )
