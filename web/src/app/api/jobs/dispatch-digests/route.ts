...
export async function GET(req: NextRequest) {
  ...
  const { data, error } = await admin
    .from("profiles")
    .select(
      "user_id, research_topics, preferred_methods, preferred_venues, digest_enabled, digest_hour_local, digest_timezone, digest_channel, digest_frequency",
    )
  ...
  for (const row of rows) {
    ...
    try {
      ...
      // Email delivery is stubbed for now — see CRON docs. In-app inbox
      // already works because the briefing_deliveries row is the inbox.
    } catch (err) {
      failed.push({
        user_id: row.user_id,
