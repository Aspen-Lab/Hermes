...
-- v2 themes store "mode:accent" composites (e.g. 'light:rose'); the old
-- enum check is obsolete. Idempotent: constraint name is the autogenerated
-- profiles_color_theme_check from the original inline check.
alter table profiles drop constraint if exists profiles_color_theme_check;
