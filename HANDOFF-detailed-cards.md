...
| P1.8 | `lib/jobs/summarize.ts` — `summarizeJob()`: 2–3 key sentences from a raw posting, no LLM | DONE | `npx vitest run src/lib/jobs/summarize.test.ts` → 1 file / 5 tests passed; 3 real-posting fixtures stay non-empty and ≤240 chars, with EEO/application noise excluded |
| P1.9 | `lib/jobs/summarize.ts` — `highlightSegments()`: split text into matched / unmatched spans | DONE | `npx vitest run src/lib/jobs/summarize.test.ts` → 1 file / 10 tests passed; exact round-trip, longest/overlap, case, boundaries, empty terms, and regex punctuation covered |
...
| P2.8 | Thread the user's location preferences into both mappers so location fit survives to the UI | DONE | `npx vitest run` → 14 files / 168 tests passed; `npx tsc --noEmit` → exit 0; preferred city = 1, unrelated on-site city = 0.4; pipeline diffs = 1 added/1 removed line each |
...
| P3.6 | Rebuild `cards/job-card.tsx` to the six-row structure, including the summary | IN_PROGRESS | |
