...
| ID | Task | Status | Verified (command + result) |
|----|------|--------|------------------------------|
| P1.1 | `lib/opportunities/salary.ts` — `parseSalaryText()` for free-text salary strings (Remotive) | IN_PROGRESS | |
| P1.2 | `lib/opportunities/salary.ts` — `normalizeSalary()` for structured min/max/currency/period inputs, with a sanity floor | TODO | |
| P1.3 | `lib/opportunities/salary.ts` — `formatSalary()` display strings | TODO | |
