...
| P7.5 | Job report renders the four sections and hides the locked block only when they render | DONE | `cd web && npx vitest run && npx tsc --noEmit` - 69 files, 571 tests passed; typecheck clean |
...
| P7.8 | Event report renders the four sections and hides the locked block only when they render | DONE | `cd web && npx vitest run && npx tsc --noEmit` - 70 files, 582 tests passed; typecheck clean |

### Phase 7D — Cost control and honesty

| ID | Task | Status | Verified (command + result) |
|----|------|--------|------------------------------|
| P7.9 | Hard cost ceiling: one call per item, token caps, zero calls without a provider | DONE | `cd web && npx vitest run src/app/api/jobs/report/route.test.ts src/app/api/events/report/route.test.ts` - 2 files, 8 tests passed; call counts 1/1/0 |
