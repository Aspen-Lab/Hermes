...
| P4.3 | Green 3-tier relevance colour on the whole card | DONE | `npx vitest run src/lib/opportunities/relevance-tier.test.ts src/lib/events/scoring.test.ts src/lib/jobs/scoring.test.ts` — 3 files, 75 tests passed; `npx tsc --noEmit` and targeted ESLint — clean; light + explicit-dark browser QA verified fixed low/medium/high tints and accent bars with unchanged text tokens and zero console errors |
| P4.4 | Wire the tab search box to filter events/jobs | DONE | `npx vitest run src/lib/opportunities/search.test.ts src/lib/opportunities/pagination.test.ts src/lib/opportunities/facets.test.ts` — 3 files, 29 tests passed; `npx tsc --noEmit`, targeted ESLint, and `git diff --check` — clean; browser QA verified All mixed search, Events `chicago` → 4, Jobs `chicago` → 3, combined facet → 0, opportunity tabs made no paper request, and no UI error state |
