...
| P8.8 | Attendee list layout — full width, no squashed column | DONE | Screenshots verified at 375 px (319 px rows, no overflow) and 1280 px (872 px rows); `cd web && npx vitest run` — 76 files / 614 tests passed with benchmark override unset |
...
| P8.11 | Strip extraction artefacts from title, subtitle and description | DONE | `cd web && npx vitest run src/lib/opportunities/` — 26 files / 237 tests passed; integrated §4 fixture removed CTA/bracket debris and rendered the visa state exactly once |
