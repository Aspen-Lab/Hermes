"""Peer CLI — entry point for all commands."""
...
PEER_DIR = Path.home() / ".peer"
DEFAULT_CONFIG = "peer.yml"
...
def main(ctx: click.Context, config: str) -> None:
    """Peer — self-hosted information agent."""
...
@main.command()
def init() -> None:
    """Initialize a new Peer project with a default config."""
    ...
    click.echo(f"Created {PEER_DIR}/")
    click.echo("\nNext: edit peer.yml, then run `peer run --once`")
...
def run(ctx: click.Context, once: bool) -> None:
    """Run the Peer pipeline."""
    import asyncio

    from peer_news.config import load_config
    from peer_news.pipeline import run_pipeline
    from peer_news.storage import Database
...
def _default_config() -> str:
    return """\
# peer.yml — Peer configuration
# Docs: https://github.com/peer-news/peer
