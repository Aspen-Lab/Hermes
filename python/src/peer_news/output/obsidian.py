...
from peer_news.output import OutputWriter
...
class ObsidianWriter(OutputWriter):
    ...
    def write(self, items: list[ScoredItem], peer_config: PeerConfig) -> Path:
        ...
        items_dir = peer_dir / "Items"
        items_dir.mkdir(exist_ok=True)
        for si in items:
            self._write_item_note(items_dir, si)

        # Write daily note
        daily_path = peer_dir / f"{date_str}.md"
        self._write_daily_note(daily_path, items, date_str)

        log.info("Wrote %d items to Obsidian vault %s", len(items), peer_dir)
        return daily_path
    ...
    def _write_daily_note(self, path: Path, items: list[ScoredItem], date_str: str) -> None:
        ...
        if self.config.frontmatter:
            ...
            lines.extend([
                "---",
                f"date: {date_str}",
                f"tags: [peer/daily, {', '.join(all_tags)}]",
                "type: peer-briefing",
                "---",
                "",
            ])

        lines.append(f"# Peer Briefing — {date_str}\n")
        ...
        for tag, tag_items in sorted(by_tag.items()):
            ...
            for si in sorted(tag_items, key=lambda x: x.score, reverse=True):
                ...
                if self.config.wikilinks:
                    lines.append(
                        f"- [[Peer/Items/{safe}|{si.item.title}]] "
    ...
    def _write_item_note(self, directory: Path, si: ScoredItem) -> None:
        ...
        if self.config.frontmatter:
            lines.extend([
                ...
                f"score: {si.score:.3f}",
                f"tags: [{', '.join(f'peer/{t}' for t in si.tags)}]",
            ])
            if si.item.authors:
