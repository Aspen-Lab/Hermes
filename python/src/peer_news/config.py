...
class PeerConfig:
    ...
    storage_path: str = "~/.peer/peer.db"
    log_level: str = "info"


def load_config(path: Path) -> PeerConfig:
    """Load and validate a YAML config file into PeerConfig."""
...
def _parse_config(raw: dict[str, Any]) -> PeerConfig:
    ...
    storage_path = storage_raw.get("path", "~/.peer/peer.db") if isinstance(storage_raw, dict) else "~/.peer/peer.db"
...
def _validate(config: PeerConfig) -> None:
